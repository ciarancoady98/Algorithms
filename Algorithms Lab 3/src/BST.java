...
public class BST<Key extends Comparable<Key>, Value> {
     ...
     * Tree height.
     *
     * Asymptotic worst-case running time using Theta notation: Theta(N) as
     * to get the height of the tree, we much recurse through each branch of the tree
     * until we get to a leaf, and then return which ever branch is the deepest.
     *
    ...
    public Key median() {
      ...
      int medianPos = ((this.root.N+1)/2);
      return select(medianPos-1);
    }
    
    private Key select(int n) {
    ...
    private String prettyPrint(Node x, String prefix) {
    	...
    	String result = prefix + "-" + x.val +"\n";
        result += prettyPrint(x.left, prefix+" |");
        result += prettyPrint(x.right, prefix+"  ");
        return result;
    }
